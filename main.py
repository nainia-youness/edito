...
def get_output_file_name(selected_folder_path):
    ...
    output_file_path = "{}/fichier_import_{}.xlsx".format(
        str(selected_folder_path), creation_time)
...
def update_output_file(output_file_df, input_file_df, variables):
    ...
    for index, _ in input_file_df.iterrows():
        ...
        # Nom du client final
        output_file_df.loc[index, output_canvas[2]
                           ] = input_file_df.loc[index, input_canvas[1]]
        # Email du client final
        output_file_df.loc[index, output_canvas[3]] = ""
        # Téléphone du client final
        output_file_df.loc[index, output_canvas[4]
                           ] = input_file_df.loc[index, input_canvas[2]]
        # Ville du client final
        output_file_df.loc[index, output_canvas[5]
                           ] = input_file_df.loc[index, input_canvas[3]]
        # Code Postal du client final
        output_file_df.loc[index, output_canvas[6]] = ""
        # Adresse du Client Final
        output_file_df.loc[index, output_canvas[7]] = "00 " + \
            input_file_df.loc[index, input_canvas[5]]
        # Modèle du colis
        ...
        output_file_df.loc[index, output_canvas[10]
                           ] = input_file_df.loc[index, input_canvas[13]]
        ...
        output_file_df.loc[index, output_canvas[12]
                           ] = input_file_df.loc[index, input_canvas[8]]
        # Référence Externe
        output_file_df.loc[index, output_canvas[13]
                           ] = input_file_df.loc[index, input_canvas[10]]
        ...
        output_file_df.loc[index, output_canvas[16]
                           ] = "Carte d'identité (Digital)"
        # Référence du retour documentaire N°2
        output_file_df.loc[index, output_canvas[17]] = "inwi1"
        # Nature
        output_file_df.loc[index, output_canvas[18]
                           ] = input_file_df.loc[index, input_canvas[0]]
        # commentaire
        output_file_df.loc[index, output_canvas[19]
                           ] = input_file_df.loc[index, input_canvas[18]]


def open_file(variables):
    Label(win, text="Veuillez ne pas fermer l'interface jusqu'à terminaison de l'exécution",
          font=('Aerial 11')).pack()
    ...
    if file:
        ...
        try:
            ...
            Label(win, text="The File is located at : {}".format(
            ...
            # select the folder where you put the output file
            selected_folder = filedialog.askdirectory()
            selected_folder_path = os.path.abspath(selected_folder)

            output_file_path = get_output_file_name(selected_folder_path)

            Label(win, text="The Folder selected is : {}".format(
                str(selected_folder_path)), font=('Aerial 11')).pack()
            output_file_df.to_excel(output_file_path, index=False)
            Label(win, text="La conversion est terminée",
                  font=('Aerial 11')).pack()
...
def show_page_2(variables):
    # Add a Label widget
    label = Label(
        win, text="Veuillez cliquer sur le bouton pour choisir le fichier", font=('Georgia 13'))
    label.pack(pady=10)

    # Create a Button
    ttk.Button(win, text="Browse", command=open_file(variables)).pack(
        pady=20)  # when you click on the button, open_file() is executed
...
def show_page_1(win):
    ...
    frame = Frame(win)
    label = Label(frame, text="Veuillez entrer vos preference",
                  font=('Georgia 13'))
    ...
    but.grid(row=len(options_list)+1, column=0,
             columnspan=2, pady=20, sticky=N+S+E+W)
