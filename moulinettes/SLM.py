...
def next_weekday(d, day):
    days_ahead_list = []
    dat = datetime.date(int(d[6:10]), int(d[3:5]), int(d[0:2])) #transforme la date de string a Date
...
SLA=pd.read_csv("./SLM_Reference.csv", sep=';', dtype="str", encoding="ISO-8859-1")
Order=pd.read_excel(r"./the_slm_planif.xlsx",dtype="str")
...
for ind in Order.index:
    for sla_ind in SLA.index:
        if Order["Ville"][ind]==SLA["LOCALITE"][sla_ind]:#pour chaque ligne , si la ville est dans notre SLA localite
            for column in SLA.columns: #parcour tout les collones du SLA de la ligne
                if SLA[column][sla_ind]=="1":# si l'un est egale a 1
                    days.append(column)# dans days on a tout les colonnes du SLA qui ont une valeur de "1" dans cette ligne
            print(Order["Date-visite"][ind])
            Order["Date-visite"][ind]=next_weekday(Order["Date-visite"][ind], days).strftime("%d/%m/%Y")
            days=[]
            count=1
    if count==0:
        ind_nv.append(ind) #ind inv on a les indices des lignes de order ou il y'a aucune ville dans le SLA"localite"
    else:
        count=0
for nv in ind_nv:
    Order["Date-visite"][nv]="Code ES non Existant"

Order.to_excel("./Ordre_passage.xlsx")